...
public class TrajectoryTesting_r3 extends Config_r3 {
    ...
    PSMotor[] psMotors = {drive.leftFront,drive.rightFront,drive.rightBack,drive.leftBack};
...
//        }
    ...
    public void loop() {
        ...
        telemetry.addData("pose.x", getEstimatedPose().heading());
        telemetry.update();
    }
    private Pose2d getEstimatedPose() {
        ...
        if (lastRotations != null) {
            ...
            Vector2d fieldPoseDelta = robotPoseDelta.rotated(Math.toRadians(gyro.getAngle()));

            estimatedPosition = estimatedPosition.added(fieldPoseDelta);
        }
        lastRotations = rotations;
        return new Pose2d(estimatedPosition, Math.toRadians(gyro.getAngle()));
    }
